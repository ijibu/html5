...
var Interfaces = {
	...
	Shadow : function () {
		var self = this,
		name = Config.spiritShadow,
		obj = Util.imgObj[name],
		imgObj = obj.obj,
		width = obj.width,
		height = obj.height,
		imgObj,
		...
		var combo_attack = function () {
			...
			var getcombo_attack = function () {
				if (_combo_attack.currFrame >= _combo_attack.framesNum) {
					...
					if (frame < _f_framesNum - _combo_attack.afterFrame)
						frame = _f_framesNum - _combo_attack.afterFrame;
					donefn && donefn();
				}
	...
	Collision : function () {
		...
		return function (w, h) {
			...
			var check = function () {
				...
				var width = rea_width || self.width,
				height = rea_height || self.height;
