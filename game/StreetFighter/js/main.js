...
var Spirit = Block.subClass(
	...
	},
	{
		init : function (f_left, f_top, direction) {
			...
			his.animate.moveto(f_left, f_top);
			return this;
		},
		...
		play : function (state, force) {
			...
			if (this.jump_combo(state) && state !== 'force_wait')
				return; //空中组合技


			if (this.defense(state))
				return; //是否在防御状态
		...
		jump_combo : function (state) { //跳跃的组合技能
			...
			if (!this.state || !this.statusManage.isJump())
				return false;
		...
		fireFrames : function () {
			...
			var _left = state.easing[0] * this.direction,
			_top = top || state.easing[1];
			...
			if (state.attack_type === 2) {
				var attack_config = state.attack_config,
				effect_position = state.effect_position;
		...
		event : function (type) {
			...
			this.animate.event.listen('frameStart', function () {
				...
				if ((self.border === 'left' && self.animateDirection[0] === -1) || (self.border === 'right' && self.animateDirection[0] === 1)) {
					...
					if (self.statusManage.get().enemy_distance_type === 'furthest')
						return;
	...
	},
	{
		...
		playAudio : function (src, type) {
			...
			if (type === 1) {
				...
				if (this.master.statusManage.isJump())
					return;
				var enemy_attack_type = this.master.enemy.statusManage.get().attack_type;
...
var WaveBoxing = Fighter.subClass(
	function (master) {
		...
		init : function () {
			...
			this.collision.event.listen('affirm', function (obj, dir) {
				...
				if (obj === self.master.enemy) {
					...
					if (Math.abs(this.top - this.master.enemy.top) > 130)
						return; //跳跃的时候降低碰撞条件
