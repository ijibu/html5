...
function onMouseClick(e) {
	if (titleIsOnFlag) {				//检查是否开始游戏
		titleIsOnFlag = false;
		startGame();
	} else if (gameEndFlag) {			//检查游戏是否结束
		if (checkPlayAgainClick(Mouse)) {		//重新开始
			gameEndFlag = false;
			endText.text = "";
			messageStr.text = "";
			scoreMessage.text = "";
	...
	} else if (gameIsPausedFlag) {		//暂停游戏
		resumeGame();
	} else if (checkGamePauseClick(Mouse)) {	//恢复游戏
		pauseGame();
	} else {
		addSpellShot(myOrigin, Mouse);		//进行射击操作
	}
}
